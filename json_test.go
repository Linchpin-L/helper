...
func TestFormatTime(t *testing.T) {
	test2 := []struct {
		...
		{"test 1", struct {
			A DateTime
		}{DateTime(time.Date(2000, 1, 2, 15, 04, 05, 0, time.Local))}, `{"A":"2000-01-02 15:04:05"}`},
		{"test 2", struct {
			A *DateTime
		}{nil}, `{"A":null}`},
		{"test 3", DateTime(time.Date(2000, 1, 1, 1, 1, 1, 0, time.Local)), `"2000-01-01 01:01:01"`},
		{"test 4", DateTime(time.Time{}), `"0001-01-01 00:00:00"`},
	}
	for _, tt := range test2 {
		t.Run(tt.name, func(t *testing.T) {
			a, e := json.Marshal(tt.s)
			if e != nil {
				t.Errorf("json.Marshal() error = %v", e)
			}
			if string(a) != tt.want {
				t.Errorf("GetFormat() = %s, want %v", a, tt.want)
			}
		})
	}
}
